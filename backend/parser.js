...
async function getAdsFromPage(url, page)
{
  ...
  // get items on page
  const ads = $('.pm-grid-item').each((i, el) => {
    //Attention, push cheerio object to array, not Element. 
    //This will allow to search inner elements in future.
    result.push($(el));
  });

  console.log(`Page ${page}: found ${ads.length}`);
  const nextPage = $('a.ty-pagination__next');
...
{
  const $ = await getPage(url);
  //get title from single page product
  const title = $('h1.ty-product-block-title bdi').text().trim();
  let district = '';
  let id = '';
  // let date = '';
  // let floor = '';
  ...
  const idParts = $('.ty-sku-item span').text();
  id = idParts;
  ...
  description = $('h1 .ty-short-description').text().trim();
  seller = $('.offer-user__details').text().trim();

  $('.ty-product-img  a img').each((i, el) => {
    const img = $(el);
...
async function run(url)
{
  ...
  for (const ad of ads) {
    if (!await askQuestion('Open this ad: ' + ad.find('.ty-grid-list__item-name a').text().trim() + '?')) {
      console.log('left: ' + (--total) + ' ads');
      continue;
    }

    let offerData = {
      price: ad.find('.ty-price-num').text().trim(),
      old_price: ad.find('.pm-old-price .ty-list-price').text().trim(),
      href:  ad.find('.ty-grid-list__item-name a').attr('href') //.replace(/\.html.*/, '.html')
    };
